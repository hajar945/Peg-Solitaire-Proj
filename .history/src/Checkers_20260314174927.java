...
public class Checkers extends JPanel {
    ...
    private static class CheckersMove {
        ...
        boolean isJump() {
            ...
            //  a jump moves exactly 2 spaces either vertically OR horizontally.
    ...
    private class Board extends JPanel implements ActionListener, MouseListener {
        ...
        void doNewGame() {
            ...
            currentPlayer = CheckersData.PEG;   // RED moves first.
            legalMoves = board.getLegalMoves(CheckersData.PEG);  // Get RED's legal moves.
        ...
        void doResign() {
            ...
            if (currentPlayer == CheckersData.PEG)
        ...
        public void paintComponent(Graphics g) {
            // tells the parent JPanel to wipe the slate clean (by painting the background color) 
            // before it starts drawing the board and pegs
        ...
        public void mousePressed(MouseEvent evt) {
            if (gameInProgress == false)
                message.setText("No more valid moves can be made! Click \"New Game\" to start a new game.");
            else {
    ...
    private static class CheckersData {
         ...
         * In Peg Solitaire, every move removes the jumped piece in the middle.
         */
        void makeMove(int fromRow, int fromCol, int toRow, int toCol) {
            // Move the peg to the new hole
            board[toRow][toCol] = board[fromRow][fromCol];
            // Empty the hole the peg started in
            board[fromRow][fromCol] = EMPTY;
            
            // ALWAYS calculate the middle hole and empty it
        ...
        /**
         * Return an array containing all the legal jumps
         * for the current board. Orthogonal jumps only!
         */
            ...
            for (int row = 0; row < 7; row++) {
                for (int col = 0; col < 7; col++) {
                    if (board[row][col] == player) {
                        ...
                        // Check UP (jump over row-1, land on row-2)
                        if (canJump(row, col, row-1, col, row-2, col))
                            moves.add(new CheckersMove(row, col, row-2, col));
                            
                        // Check DOWN (jump over row+1, land on row+2)
                        if (canJump(row, col, row+1, col, row+2, col))
                            moves.add(new CheckersMove(row, col, row+2, col));
                            
                        // Check LEFT (jump over col-1, land on col-2)
                        if (canJump(row, col, row, col-1, row, col-2))
                            moves.add(new CheckersMove(row, col, row, col-2));
                            
                        // Check RIGHT (jump over col+1, land on col+2)
                        if (canJump(row, col, row, col+1, row, col+2))
                            moves.add(new CheckersMove(row, col, row, col+2));
                    }
                }
        ...
        /* 
         * Return a list of the legal jumps that can be made
        ...
        CheckersMove[] getLegalJumpsFrom(int player, int row, int col) {
            ...
            if (board[row][col] == player) {
                if (canJump(row, col, row-1, col, row-2, col)) // UP
                    moves.add(new CheckersMove(row, col, row-2, col));
                if (canJump(row, col, row+1, col, row+2, col)) // DOWN
                    moves.add(new CheckersMove(row, col, row+2, col));
                if (canJump(row, col, row, col-1, row, col-2)) // LEFT
                    moves.add(new CheckersMove(row, col, row, col-2));
                if (canJump(row, col, row, col+1, row, col+2)) // RIGHT
                    moves.add(new CheckersMove(row, col, row, col+2));
            }
         ...
         * r1,c1 = start | r2,c2 = peg being jumped | r3,c3 = landing hole
         */
        private boolean canJump(int r1, int c1, int r2, int c2, int r3, int c3) {
            // 1. Is the landing spot off the board? 
            if (r3 < 0 || r3 >= 7 || c3 < 0 || c3 >= 7)
                return false; 

            // 2. Is the landing spot truly empty? (Ensures we don't land on INVALID corners)
            if (board[r3][c3] != EMPTY)
                return false; 

            // 3. Is there actually a peg in the middle to jump over?
            if (board[r2][c2] != PEG)
                return false; 

            // If it passes all 3 tests, it's a legal Peg Solitaire jump 
            return true; 
        }  // end canJump()
