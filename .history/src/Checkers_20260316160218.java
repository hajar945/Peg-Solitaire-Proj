Custom board sizes are a fantastic idea! A standard English board is a 7x7 grid, but expanding it to 9x9 or even 11x11 makes the puzzle much harder.

To make the board size dynamic, we have to change every hardcoded `7` in your code into a variable (like `boardSize`). 

We also need to use a little math to dynamically calculate where the "corners" and the "center" are, no matter how big the board gets.
* **The Center:** Can always be found at `boardSize / 2`.
* **The Corners:** On a cross-shaped board, the arms usually take up the middle third. So, the corner size is `boardSize / 3`.

I have updated your entire script. It completely removes the leftover Checkers `canMove` logic you had trapped at the bottom, dynamically draws the board size, and adds a brand new **Change Size** button that prompts the user for a valid odd number. 

Highlight your entire file and replace it with this:
...
public class Checkers extends JPanel {
    ...
    public Checkers() {
        setLayout(null);  
        // Increased the window size to comfortably fit boards up to 13x13
        ...
        // Gave the board a larger dedicated area so it can grow
        board.setBounds(20, 20, 280, 280); 
        
        newGameButton.setBounds(310, 60, 120, 30);
        resignButton.setBounds(310, 100, 120, 30);
        sizeButton.setBounds(310, 140, 120, 30); // Positioned below Resign
        message.setBounds(0, 310, 450, 30);
    } 
    ...
    private class Board extends JPanel implements ActionListener, MouseListener {
        ...
        public void paintComponent(Graphics g) {
            ...
            // Replaced hardcoded 7s with board.boardSize
            for (int row = 0; row < board.boardSize; row++) {
                for (int col = 0; col < board.boardSize; col++) {
                    ...
                    case CheckersData.EMPTY:
                        ...
                        g.fillOval(8 + col*20, 8 + row*20, 7, 7); 
                        break;
                    }
                }
            }
        ...
        public void mousePressed(MouseEvent evt) {
            if (gameInProgress == false)
                ...
                // Using board.boardSize to establish clickable bounds dynamically
...
Since the pegs are still a little small (20x20 pixels), would you like me to show you how to multiply the visual scale so the board takes up more of the screen?